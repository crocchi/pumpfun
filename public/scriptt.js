...
socket.on('stats', (data) => {
 console.log('📊 Statistiche aggiornate:', data);
  ...
  document.getElementById('otherToken').textContent = `BTC [$ ${Number(data.btc.price).toFixed()}] `;
  ...
  document.getElementById('btcpriceup').textContent = `1h[${data.btc.percent_change_1h}%] 24h[${data.btc.percent_change_24h?.toFixed(2)}%] `;
})
      ...
      playScale();
        ...
        searchEl.cells[8].innerHTML = `LiqDrop[${token.stats?.liqDrop?.toFixed(2) || 'N/A'}] Speed[${token.stats?.speedLiq?.toFixed(2) || 'N/A'}] Trend[${token.stats?.liqTrend?.toFixed(2) || 'N/A'}] Trx 1/s[${token.stats?.tradesPerSec?.toFixed(2) || 'N/A'}] Trx/m[${token.stats?.tradesPerMin || 'N/A'}] Life[${token.stats?.tokenLifeSec || 'N/A'} s] Volat[${token.stats?.volatility?.toFixed(2) || 'N/A'}] `;
			} else {
				// Add a new row for the token
				let html = `<tr id='mon${token.id}'>
								...
								<td>${token.time}</td>
								<td>${defiExchange}</td>
								<td >LiqDrop[${token.stats?.liqDrop?.toFixed(2) || 'N/A'}] Speed[${token.stats?.speedLiq?.toFixed(2) || 'N/A'}] Trend[${token.stats?.liqTrend?.toFixed(2) || 'N/A'}] Trx 1/s[${token.stats?.tradesPerSec?.toFixed(2) || 'N/A'}] Trx/m[${token.stats?.tradesPerMin || 'N/A'}] Life[${token.stats?.tokenLifeSec || 'N/A'} s] Volat[${token.stats?.volatility?.toFixed(2) || 'N/A'}] </td>
